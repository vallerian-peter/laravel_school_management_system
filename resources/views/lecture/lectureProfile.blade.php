        ...
        <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
          ...
          <li class="nav-item">
            <a href="lectureAllStudents" class="nav-link">
              ...
              <p>
                All Students
                <span class="right badge badge-primary">{{ $lecture->module->department->course->student->count() }}</span>
              </p>
            </a>
          </li>

          <li class="nav-item">
            <a href="lectureProfile" class="nav-link active">
  ...
  <div class="content-wrapper">
    ...
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1 class="m-0"> {{ $lecture->username }} Profile</h1>
          </div><!-- /.col -->
          <div class="col-sm-6">
         ...
         <div class="row">
            <div class="col-md-8">
                ...
                <div class="card px-3 pt-3 pb-4">
                    <div class="h2">Lecture Avatar</div>
                    <p>This avatar profile is default assigned Alexandar image, by Updating it will change it to your image</p>
                    
                    <div class="avatar-img">
                        <div class="row">
                            <div class="col-md-4"  style="display: flex; flex-direction: column; align-items:center; justify-content:center;">
                              <div id="imagePreview">
                                <img id="previewImg" src="{{ $lecture->avatar ? asset('storage/' . $lecture->avatar) : asset('AdminLTE-3.2.0/AdminLTE-3.2.0/dist/img/user2-160x160.jpg')}}" width="150px" height="150px" class="brand-image img-circle elevation-3" style="opacity: .8; object-fit:cover;">
                              </div>
                            </div>
                            <div class="col-md-8">
                              <form action="lectureUpdateAvatar" method="POST" enctype="multipart/form-data">
                                @csrf
                                <div class="mb-3">
                                    <label for="avatarInput" class="form-label">Update Avatar</label>
                                    <input type="file" id="avatarInput" class="form-control" name="avatar" accept="image/*" onchange="previewImage(this, {{ json_encode($lecture->avatar) }})">
                ...
                <div class="py-3">
                    <div class="card px-3 py-4">
                        <div class="h2">Lecture Details</div>
                        <p>This details are your credentials from the database by changing </p>
                         
                        {{-- <div class="h3">Update Details</div> --}}
                        <form action="lectureUpdateDetails" method="POST">
                            @csrf
                            <div class="mb-3">
                                <label for="fullname" class="form-label">Full Name</label>
                                <input type="text" class="form-control" name="fullname" value="{{ $lecture->fullname }}">
                                <span class="text-danger"> @error('fullname') {{ $message }} @enderror </span>
                            </div>
                            <div class="mb-3">
                                <label for="email" class="form-label">Email</label>
                                <input type="email" class="form-control" name="email" value="{{ $lecture->email }}">
                                <span class="text-danger"> @error('email') {{ $message }} @enderror </span>
                            </div>
                            <div class="mb-3">
                                <label for="username" class="form-label">Username</label>
                                <input type="username" class="form-control" name="username" value="{{ $lecture->username }}">
                                <span class="text-danger"> @error('username') {{ $message }} @enderror </span>
                            </div>
                            <div class="mb-3">
                                <label for="phone" class="form-label">Phone</label>
                                <input type="phone" class="form-control" name="phone" value="{{ $lecture->phone }}">
                ...
                <div class="py-3">
                    <div class="card px-3 py-3">
                        <div class="h2">Lecture Modules</div>
                        <p>This lecture Modules is the module you choose from the sign-up, changing this will cause the to all system.
                       <br> Your Module: <span class="h6 fw-bold text-primary">{{ $lecture->module->name }}</span> </span> </p>

                        <form action="lectureUpdateModule" method="POST">
                            @csrf
                            <div class="mb-3">
                                <label for="module" class="form-label">Update module</label>
                ...
                <div class="py-3">
                    <div class="card px-3 py-3">
                        <div class="h2"> <i class="bi bi-link text-success"></i> Update Password</div>
                        <p>Update password in your account is great, by changing randomly password to another enhance security.</p>    
                        
                        <form action="lectureUpdatePassword" method="POST">
                             @csrf
                             <div class="mb-3">
                                <label for="current_password" class="form-label">Current Password</label>
                                <input type="password" class="form-control" name="currentpassword" value="{{ @old('currentpassword') }}" id="current_password" placeholder="Current Password">
                                <span class="text-danger"> @error('currentpassword') {{ $message }} @enderror</span>
                             </div>
                             <div class="mb-3">
                                <label for="new_password" class="form-label">New Password</label>
                                <input type="password" class="form-control" name="newpassword" value="{{ @old('newpassword') }}" id="new_password" oninput="validatePassword()" placeholder="New Password">
                                <span class="text-danger"> @error('newpassword') {{ $message }} @enderror</span>
                             </div>
                             <div class="mb-3">
                                <label for="confirm_password" class="form-label">Confirm Password</label>
                                <input type="text" class="form-control" name="confirmpassword" id="confirm_password" oninput="validatePassword()" placeholder="Confirm Password">
                                <span class="text-danger" id="confirmPassMessage"> @error('confirmpassword') {{ $message }} @enderror</span>
                             </div>
                             <div class="mb-3">
                                <button type="submit" class="btn btn-dark">Save</button>
                             </div>

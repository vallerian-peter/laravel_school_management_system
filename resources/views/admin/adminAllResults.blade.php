...
<head>
  ...
  <meta http-equiv="pragma" content="no-cashe">
    ...
    <title>Admin | All-Departments</title>
</head>
<body>
    ...
    <div class="sidebar">
      ...
      <nav class="mt-2">
        <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
          ...
          <li class="nav-item">
            <a href="adminAllResults" class="nav-link active">
                <i class="far fa-circle nav-icon"></i>
              <p>
                All Results
  ...
  <div class="content-wrapper">
    ...
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1 class="m-0"> {{ $admin->username }} | All Results</h1>
          </div><!-- /.col -->
          <div class="col-sm-6">
      ...
      <div class="container-fluid">
        ...
        <div class="row py-4">
            <div class="col-md-12">
                <div class="card p-3">
                    <p>
                        This are all Students results uploaded from Lectures Modules from there semister
                    </p>
                    ...
                    <div class="table-responsive">
                        <div class="d-flex" style="display: flex; align-items:center; justify-content:space-between;">
                           ...
                           <div class="float-right d-flex">
                              <button type="submit" class="btn btn-primary btn-sm m-2" data-bs-toggle="modal" data-bs-target="#AdminAddResult">+Add Result</button>
                            </div>
                        </div>
                            ...
                            <tbody id="tbody">
                                ...
                                @foreach ($results as $result)
                                    <tr>
                                        ...
                                        <td>{{ $result->module->lecture->fullname }}</td>
                                        <td>{{ $result->module->name }}</td>
                                        <td>{{ $result->module->semester_year }}</td>
                                        <td>{{ $result->ca }}</td>
                                        <td>{{ $result->fe }}</td>
                                        <td>{{ $result->marks }}</td>
                                        <td>{{ $result->grade }}</td>
                                        <td>{{ $result->status }}</td>
                                        <td>
                                          <button type="button" class="btn btn-outline-success btn-sm text-center" data-bs-target="#AdminUpdateResult{{ $result->id }}" data-bs-toggle="modal"> <i class="bi bi-pencil"></i> </button> 
                                          <a href="{{ route('admin.delete.result', $result->id) }}" class="btn btn-outline-danger btn-sm text-center"> <i class="bi bi-trash"></i> </a>
                                        </td>
                                    </tr>
                                @endforeach
                        ...
                        <div class="d-flex" style="display: flex; align-items:center; justify-content:flex-end;">
                            <div class="box-count py-3 text-muted">
                                All Results: <span id="countMod"> {{ $results->count() }} </span>
                            </div>
                        </div>
                    </div>
                </div>
...
{{-- ADMIN ADD COURSE --}}
  <div class="modal fade" id="AdminAddResult" tabindex="-1" aria-labelledby="AdminAddResultLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
        <div class="modal-header" style="display:flex; align-items:center;">
          <div class="left" style="display: flex; flex-direction: column;">
            <h1 class="modal-title fs-5" id="AdminAddResultLabel">Admin | Add-Result</h1>
          </div>
          <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
        </div>
        <div class="modal-body">
            <form action="AdminAddResult" method="POST">
                @csrf
                <div class="mb-3">
                    <label for="text" class="form-label">Result Name</label>
                    <input type="text" id="text" name="name" class="form-control" placeholder="Result Name">
                    <span class="text-danger"> @error('name') {{ $message }} @enderror </span>
                </div>
                
                <div class="text-center w-100 py-3">
                    <button type="submit" class="btn btn-dark w-50 text-center">Add Result</button>
                </div>
  ...
  {{-- ADMIN UPDATE COURSE --}}
  @foreach($results as $result)
  <div class="modal fade" id="AdminUpdateResult{{ $result->id }}" tabindex="-1" aria-labelledby="AdminUpdateResult{{ $result->id }}Label" aria-hidden="true">
    <div class="modal-dialog modal-lg">
      <div class="modal-content">
        <div class="modal-header" style="display:flex; align-items:center;">
          <div class="left" style="display: flex; flex-direction: column;">
            <h1 class="modal-title fs-5" id="AdminUpdateResult{{ $result->id }}Label">Admin | Update-dep</h1>
          </div>
          <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
        </div>
        <div class="modal-body">
            <form action="AdminUpdateResult" method="POST">
                @csrf
                <input type="text" name="id" value="{{ $result->ca }}" hidden>
                <div class="mb-3">
                    <label for="text" class="form-label">FE</label>
                    <input type="text" id="text" name="name" class="form-control" value="{{ $result->fe }}" placeholder="Result Name">
                    <span class="text-danger"> @error('name') {{ $message }} @enderror </span>
                </div>
                
                <div class="text-center w-100 py-3">
                    <button type="submit" class="btn btn-dark w-50 text-center">Update Result</button>
                </div>

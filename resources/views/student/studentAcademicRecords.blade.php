      ...
      <div class="container-fluid">
       ...
       <div class="row">
            <div class="col-md-8">
                @php
                    $currentYear = date('Y');
                    $studentRegYr = $student->created_at->format('Y');
                    $Coursedur = (int) Str::before($student->course->year_duration, 'years');
                    $dif = ($studentRegYr - $currentYear) + 1;
                @endphp

                  @for ($i = 0; $i < $Coursedur; $i++)
                      @if ($dif >= ($i+1))
                        <a href="{{ route('student.results.display', ['resultId'=>($i+1)]) }}" class="card p-3 d-flex">
                          <div class="py-1 fw-bold h5"> 
                          ...
                          <div class="py-1"> {{ $studentRegYr + $i }} {{ '-' }} {{ $studentRegYr + ($i + 1) }} </div>
                        </a>
                      @else
                        <a href="#" class="card p-3 d-flex" style="pointer-events: none; opacity: 0.5;">
                          <div class="py-1 fw-bold h5"> 
                            @if ($i == 0)
                                {{ 'First' }}
                            @elseif($i == 1)

    ...
    <div class="sidebar">
      ...
      <nav class="mt-2">
        <ul class="nav nav-pills nav-sidebar flex-column" data-widget="treeview" role="menu" data-accordion="false">
          ...
          <li class="nav-item">
            <a href="studentCourseModules" class="nav-link active">
  ...
  <div class="content-wrapper">
    ...
    <div class="content-header">
      <div class="container-fluid">
        <div class="row mb-2">
          <div class="col-sm-6">
            <h1 class="m-0"> {{ $student->username }} Course-Modules</h1>
          </div><!-- /.col -->
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              <li class="breadcrumb-item"><a href="studentDashboard">Home</a></li>
              <li class="breadcrumb-item">Course-Modules</li>
            </ol>
          </div><!-- /.col -->
        </div><!-- /.row -->
      </div><!-- /.container-fluid -->
      ...
      <div class="container-fluid">
        ...
        <div class="row py-4">
            <div class="col-md-10">
                <div class="card p-3">
                    <p>
                        This are your modules from different yeaars from the current course selected <span class="fw-bold text-primary">{{ $student->course->name }}</span>
                    </p>
                    ...
                    <div class="table-responsive">
                        <div class="d-flex" style="display: flex; align-items:center; justify-content:space-between;">
                           ...
                           <div>
                            <select name="filter" id="filter" width="10px" class="form-control">
                                <option value="1S1" selected>1yr Sem1</option>
                        ...
                        <table class="table table-responsive table-bordered table-striped table-hover">
                            ...
                            <tbody id="tbody">
                                ...
                                @foreach ($modules->where('semester_year', '1S1') as $module)
                                    <tr>
                                        <th rowspan="1"> <?=$id++;?> </th>
                                        <td>{{ $module->name }}</td>
                        ...
                        <div class="d-flex" style="display: flex; align-items:center; justify-content:flex-end;">
                            <div class="box-count py-3 text-muted">
                                All Modules: <span id="countMod"> {{ $modules->where('semester_year', '1S1')->count() }} </span>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                if(count != 0)
                {
                  for(var i = 0; i < output.length; i++) {
                      ...
                      htmlData += '<tr>\
                                      <th rowspan="1">'+(i+1)+'</th>\
                                      <td>'+output[i]['name']+'</td>\
                                      <td>'+output[i]['module_code']+'</td>\
                                      <td>'+output[i]['credit']+'</td>\
                                      <td>'+output[i]['semester_year']+'</td>\
                                  \</tr>';
                  }

                }else
                {
                     htmlData += '<tr colspan="5">\
                                      <td colspan="5" class="text-center text-danger"> <i class="bi bi-trash"></i> Empty Modules</td>\
                                  \</tr>';
                }

                $('#tbody').html(htmlData);
                $('#countModules').html(count);
                $('#countMod').html(count);
              }
          });
      });
